...
// 전역 fetch 함수를 모의(mock)하여 실제 API 호출을 방지합니다.
...
describe("App Component", () => {
  ...
  it('should display "Loading images..." initially', () => {
    // API 호출이 아직 해결되지 않은 Promise를 반환하도록 모의
  ...
  // 테스트 3: 이미지 URL이 없을 때 "No images found."를 표시하는지 확인
  it('should display "No images found." when no images are present', async () => {
    // 모의 fetch 함수가 빈 배열을 반환하도록 설정
    mockFetch.mockResolvedValueOnce({
      ok: true,
  ...
  it("should display an error message on API fetch failure", async () => {
    ...
    expect(
      screen.getByText(
        "Error: Failed to load images: Failed to fetch images from backend."
      )
    ).toBeInTheDocument();
  });
});
